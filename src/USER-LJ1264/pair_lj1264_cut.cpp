...
PairLJ1264Cut::PairLJ1264Cut(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 1;
  writedata = 1;
...
void PairLJ1264Cut::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,r2inv,r4inv,r6inv,forcelj,factor_lj;
...
void PairLJ1264Cut::compute_inner()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,r2inv,r4inv,r6inv,forcelj,factor_lj,rsw;
  int *ilist,*jlist,*numneigh,**firstneigh;
...
void PairLJ1264Cut::compute_middle()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,r2inv,r4inv,r6inv,forcelj,factor_lj,rsw;
...
void PairLJ1264Cut::compute_outer(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,r2inv,r4inv,r6inv,forcelj,factor_lj,rsw;
...
void PairLJ1264Cut::allocate()
{
  ...
  memory->create(lj4c4,n+1,n+1,"pair:lj4c4");
...
void PairLJ1264Cut::settings(int narg, char **arg)
{
...
double PairLJ1264Cut::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJ1264Cut::write_restart(FILE *fp)
{
...
void PairLJ1264Cut::read_restart(FILE *fp)
{
...
void PairLJ1264Cut::write_restart_settings(FILE *fp)
{
...
void PairLJ1264Cut::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairLJ1264Cut::write_data(FILE *fp)
{
...
void PairLJ1264Cut::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp,"%d %d %g %g %g %g\n",i,j,epsilon[i][j],sigma[i][j],c4[i][j],cut[i][j]);
}

/* ---------------------------------------------------------------------- */

double PairLJ1264Cut::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
                         double /*factor_coul*/, double factor_lj,
                         double &fforce)
{
  double r2inv,r4inv,r6inv,forcelj,philj;
