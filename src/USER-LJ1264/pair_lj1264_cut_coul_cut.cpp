...
#include "pair_lj1264_cut_coul_cut.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include "atom.h"
...
PairLJ1264CutCoulCut::PairLJ1264CutCoulCut(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
...
PairLJ1264CutCoulCut::~PairLJ1264CutCoulCut()
{
  if (allocated) {
...
void PairLJ1264CutCoulCut::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul,fpair;
  double rsq,r2inv,r4inv,r6inv,forcecoul,forcelj,factor_coul,factor_lj;
...
void PairLJ1264CutCoulCut::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJ1264CutCoulCut::settings(int narg, char **arg)
{
...
void PairLJ1264CutCoulCut::init_style()
{
  if (!atom->q_flag)
...
double PairLJ1264CutCoulCut::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJ1264CutCoulCut::write_restart(FILE *fp)
{
...
void PairLJ1264CutCoulCut::read_restart(FILE *fp)
{
...
void PairLJ1264CutCoulCut::write_restart_settings(FILE *fp)
{
...
void PairLJ1264CutCoulCut::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJ1264CutCoulCut::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairLJ1264CutCoulCut::single(int i, int j, int itype, int jtype,
                                double rsq,
...
void *PairLJ1264CutCoulCut::extract(const char *str, int &dim)
{
  dim = 2;
